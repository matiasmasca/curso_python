...
# Que pasa cuando la pelota toca el fondo?
# debemos detectar que toco el fondo.
# Frenar la pelota y volver a setear para el saque.
...
while True:
    ...
    # Tarea-2: a la tercer vida pierda, reiniciar el tablero.
    if (pelota_y >= 600 - 8):
        pelotaEnMovimiento = False
        pelota_x, pelota_y = (24, pelotaEmpiezaEnY)
        pelotaVelocidad = 3
    ...
    if indiceLadrilloTocado >= 0:
        ladrilloTocado = ladrillos[indiceLadrilloTocado]
        ...
        if medio_ladrillo_x > ladrilloTocado.x + ladrilloTocado.width or medio_ladrillo_x < ladrilloTocado.x:
            ...
            velocidad_y *= -1
        del (ladrillos[indiceLadrilloTocado]) # borramos el ladrillo tocado de la lista
